...
sim_ave <- sim_all %>%
  ...
  summarize(rank_L = mean(rank_L),
            density_S = mean(density_S),
            L_rekl_error = mean(L_rel_error))

sim_min <- 
  sim_all %>%
  group_by(lam_values, mu_values) %>% 
  filter(min(L_rel_error) == L_rel_error)

sim_max <- 
  sim_all %>%
  group_by(lam_values, mu_values) %>% 
  filter(max(L_rel_error) == L_rel_error)
