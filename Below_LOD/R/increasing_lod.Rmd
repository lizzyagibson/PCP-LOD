...
F_norm %>% ggplot(aes(x = percent_blod, y = norm)) + 
  ...
  labs(x = "Percent Below LOD", y = "Relative Error (norm(X-L-S) / norm(X))")
```
...
rbind(L_diff, S_diff) %>% ggplot(aes(x = percent_blod, y = norm)) + 
  ...
  labs(x = "Percent Below LOD", y = "Relative Error in Solution Matrices (norm(difference) / norm(lod0))")
```
...
V_diff <- as_tibble(cbind(lod_orig = norm((V_original - V_lod0), type = "F")/norm((V_lod0), type = "F"),
      lod0 =norm((V_lod0 - V_lod0), type = "F")/norm((V_lod0), type = "F"),
...
colSums(blod_10 != 0)/nrow(blod_10)

tf_lod_20 <- mix_data_lod_20 %>% 
  mutate_all(~ifelse(. == -1, TRUE, FALSE))
blod_20 <- as.matrix(tf_lod_20*L_lod20)
true_20 <- as.matrix(mixture_data*tf_lod_20)
colSums(blod_20 != 0)/nrow(blod_20)

tf_lod_30 <- mix_data_lod_30 %>% 
  mutate_all(~ifelse(. == -1, TRUE, FALSE))
blod_30 <- as.matrix(tf_lod_30*L_lod30)
true_30 <- as.matrix(mixture_data*tf_lod_30)
colSums(blod_30 != 0)/nrow(blod_30)

tf_lod_40 <- mix_data_lod_40 %>% 
  mutate_all(~ifelse(. == -1, TRUE, FALSE))
blod_40 <- as.matrix(tf_lod_40*L_lod40)
true_40 <- as.matrix(mixture_data*tf_lod_40)
colSums(blod_40 != 0)/nrow(blod_40)

tf_lod_50 <- mix_data_lod_50 %>% 
  mutate_all(~ifelse(. == -1, TRUE, FALSE))
blod_50 <- as.matrix(tf_lod_50*L_lod50)
true_50 <- as.matrix(mixture_data*tf_lod_50) 
colSums(blod_50 != 0)/nrow(blod_50)
```

```{r}
#divide by range
less_diff <- as_tibble(cbind(lod10 = norm((blod_10 - true_10)/(range(blod_10)[1] - range(blod_10)[2]), type = "F"),
      lod20 = norm((blod_20 - true_20)/(range(blod_20)[1] - range(blod_20)[2]), type = "F"),
      lod30 = norm((blod_30 - true_30)/(range(blod_30)[1] - range(blod_30)[2]), type = "F"),
      lod40 = norm((blod_40 - true_40)/(range(blod_40)[1] - range(blod_40)[2]), type = "F"),
      lod50 = norm((blod_50 - true_50)/(range(blod_50)[1] - range(blod_50)[2]), type = "F"))) %>% 
  gather(percent_blod, norm)

less_diff %>% ggplot(aes(x = percent_blod, y = norm)) + geom_point() + geom_path(aes(group = 1)) + theme_bw()

#divide by variance
less_diff2 <- as_tibble(cbind(lod10 = norm((blod_10 - true_10)/var(as.vector(blod_10)), type = "F"),
      lod20 = norm((blod_20 - true_20)/var(as.vector(blod_20)), type = "F"),
      lod30 = norm((blod_30 - true_30)/var(as.vector(blod_30)), type = "F"),
      lod40 = norm((blod_40 - true_40)/var(as.vector(blod_40)), type = "F"),
      lod50 = norm((blod_50 - true_50)/var(as.vector(blod_50)), type = "F"))) %>% 
  gather(percent_blod, norm)
...
above_diff <- as_tibble(cbind(
  lod10 = norm((above_blod_10 - above_true_10)/(range(above_blod_10)[1] - range(above_blod_10)[2]), type = "F"),
  lod20 = norm((above_blod_20 - above_true_20)/(range(above_blod_20)[1] - range(above_blod_20)[2]), type = "F"),
  lod30 = norm((above_blod_30 - above_true_30)/(range(above_blod_30)[1] - range(above_blod_30)[2]), type = "F"),
  lod40 = norm((above_blod_40 - above_true_40)/(range(above_blod_40)[1] - range(above_blod_40)[2]), type = "F"),
  lod50 = norm((above_blod_50 - above_true_50)/(range(above_blod_50)[1] - range(above_blod_50)[2]), type = "F"))) %>% 
  gather(percent_blod, norm)
...
above_diff2 <- as_tibble(cbind(
  lod10 = norm((above_blod_10 - above_true_10)/var(as.vector(above_blod_10)), type = "F"),
  lod20 = norm((above_blod_20 - above_true_20)/var(as.vector(above_blod_20)), type = "F"),
  lod30 = norm((above_blod_30 - above_true_30)/var(as.vector(above_blod_30)), type = "F"),
  lod40 = norm((above_blod_40 - above_true_40)/var(as.vector(above_blod_40)), type = "F"),
  lod50 = norm((above_blod_50 - above_true_50)/var(as.vector(above_blod_50)), type = "F"))) %>% 
  gather(percent_blod, norm)
