...
rbind(F_norm_2, F_norm_sqrt2, F_norm_0, F_norm) %>% ggplot(aes(x = percent_blod, y = norm)) + 
...
#Subtract true values and divide by norm of true
nn_less_diff <- as_tibble(cbind(`0%` = norm((blod_0 - nn_l_blod_0 - nn_s_blod_0), type = "F"), #/norm((mix), type = "F"),
      `10%` = norm((blod_10 - nn_l_blod_10 - nn_s_blod_10), type = "F")/norm((blod_10), type = "F"),
      `20%` = norm((blod_20 - nn_l_blod_20 - nn_s_blod_20), type = "F")/norm((blod_20), type = "F"),
      `30%` = norm((blod_30 - nn_l_blod_30 - nn_s_blod_30), type = "F")/norm((blod_30), type = "F"),
      `40%` = norm((blod_40 - nn_l_blod_40 - nn_s_blod_40), type = "F")/norm((blod_40), type = "F"),
      `50%` = norm((blod_50 - nn_l_blod_50 - nn_s_blod_50), type = "F")/norm((blod_50), type = "F"))) %>% 
  gather(percent_blod, norm) %>% 
  mutate(Values = "< LOD")
...
nn_above_diff <- as_tibble(cbind(
  `0%`  = norm((mix  - nn_l_above_blod_0  - nn_s_above_blod_0),  type = "F")/norm(mix,  type = "F"),
  `10%` = norm((mix_data_lod_10_0 - nn_l_above_blod_10 - nn_s_above_blod_10), type = "F")/norm(mix_data_lod_10_0, type = "F"),
  `20%` = norm((mix_data_lod_20_0 - nn_l_above_blod_20 - nn_s_above_blod_20), type = "F")/norm(mix_data_lod_20_0, type = "F"),
  `30%` = norm((mix_data_lod_30_0 - nn_l_above_blod_30 - nn_s_above_blod_30), type = "F")/norm(mix_data_lod_30_0, type = "F"),
  `40%` = norm((mix_data_lod_40_0 - nn_l_above_blod_40 - nn_s_above_blod_40), type = "F")/norm(mix_data_lod_40_0, type = "F"),
  `50%` = norm((mix_data_lod_50_0 - nn_l_above_blod_50 - nn_s_above_blod_50), type = "F")/norm(mix_data_lod_50_0, type = "F"))) %>% 
  gather(percent_blod, norm) %>% 
  mutate(Values = "> LOD")
...
rbind(V_diff,
      V_diff_0,
      V_diff_sqrt2,
      V_diff_2) %>% ggplot(aes(x = percent_blod, y = norm, group = Method, color = Method)) + 
...
rbind(u_diff,
      u_diff_0,
      u_diff_sqrt2,
      u_diff_2) %>% ggplot(aes(x = percent_blod, y = norm, group = Method, color = Method)) + 
  geom_point() + geom_path() + theme_bw() +
  labs(x = "Percent Below LOD", 
       y = "norm(SV - SV_lod0) / norm(SV_lod0)",
...
rbind(d_diff_sqrt2,
      d_diff,
      d_diff_0,
      d_diff_2) %>% ggplot(aes(x = percent_blod, y = norm, group = Method, color = Method)) + 
  geom_point() + geom_path() + theme_bw() +
  labs(x = "Percent Below LOD", 
       y = "norm(SV - SV_lod0) / norm(SV_lod0)",
...
plotall %>% 
  ...
  geom_point(aes(color = Compare), alpha = 0.25) + 
  theme_bw() +
  facet_wrap(.~Percent) +
  labs(x = "Individual", 
       y = "Solution Value",
...
plotall %>% 
  ...
  geom_jitter(aes(color = Compare), alpha = 0.25, width = 0.3, height = 0) + 
  theme_bw() +
  facet_wrap(.~Percent) +
  labs(x = "POP", 
       y = "Solution Value",
...
plotall %>% 
  ...
  mutate(Value_SD = Value - LOD,
         Original_SD = Original - LOD) %>% 
  ggplot(aes(x = Original_SD, y = Value_SD, 
             color = Percent)) + 
  geom_point(alpha = 0.25) + theme_bw() +
  geom_abline(intercept = 0, slope = 1) +
