...
out_f %>% 
  ...
  summarize(q25 = quantile(norm, probs = .05),
    Mean = mean(norm),
    q75 = quantile(norm, probs = .95)) %>% 
  ggplot(aes(x = percent_blod, y = Mean, group = Method, ymin = q25, ymax = q75, color = Method)) + 
    geom_point() + 
...
out_ls %>% 
  ...
  summarize(q25 = quantile(norm, probs = .05),
            Mean = mean(norm),
            q75 = quantile(norm, probs = .95)) %>% 
  ggplot(aes(x = percent_blod, y = Mean, group = interaction(Method,Matrix), 
...
out_ab %>% 
  ...
  ggplot(aes(x = percent_blod, y = Mean, group = interaction(Values, Method), color = Method, ymin = q25, ymax = q75)) + 
    geom_point() + 
  geom_errorbar(width=0.1) +
...
out_v %>% 
  ...
  summarize(q25 = quantile(norm, probs = .05),
            Mean = mean(norm),
            q75 = quantile(norm, probs = .95)) %>% 
  ggplot(aes(x = percent_blod, y = Mean, ymin = q25, ymax = q75, group = Method, color = Method)) + 
    geom_point() + 
  geom_errorbar(width=0.1) +
  geom_path(aes(color = Method)) + theme_bw() +
  labs(x = "Percent Below LOD", y = "norm(X-L-S) / norm(X)",
