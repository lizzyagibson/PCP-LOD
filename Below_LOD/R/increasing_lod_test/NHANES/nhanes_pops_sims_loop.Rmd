...
out_f %>% 
  ...
  summarize(q25 = quantile(norm, probs = .25),
    Median = median(norm),
    q75 = quantile(norm, probs = .75)) %>% 
  ggplot(aes(x = percent_blod, y = Median, group = Method, ymin = q25, ymax = q75)) + 
  geom_pointrange(aes(color = Method), size = 0.5) + geom_path(aes(color = Method)) + theme_bw() +
  labs(x = "Percent Below LOD", y = "norm(X-L-S) / norm(X)",
       title = "Relative Prediction Error")
```
...
out_ls %>% 
  ...
  summarize(q25 = quantile(norm, probs = .25),
            Median = median(norm),
            q75 = quantile(norm, probs = .75)) %>% 
  ggplot(aes(x = percent_blod, y = Median, group = interaction(Method,Matrix), ymin = q25, ymax = q75)) + 
  geom_pointrange(aes(color = Method), size = 0.5) + geom_path(aes(linetype = Matrix, color = Method)) + theme_bw() +
  labs(x = "Percent Below LOD", y = "norm(difference) / norm(lod0))",
       title = "Relative Error in Solution Matrices")
```
...
out_ab %>% 
  ...
  geom_pointrange(size = 0.5) + geom_path(aes(linetype = Values)) + theme_bw() +
  labs(x = "Percent Below LOD", 
       y = "(norm(X-L-S) / norm(X)",
       title = "Relative Error in Values < LOD & > LOD")
```
...
out_v %>% 
  ...
  summarize(q25 = quantile(norm, probs = .25),
            Median = median(norm),
            q75 = quantile(norm, probs = .75)) %>% 
  ggplot(aes(x = percent_blod, y = Median, ymin = q25, ymax = q75, group = Method)) + 
  geom_pointrange(aes(color = Method), size = 0.5) + geom_path(aes(color = Method)) + theme_bw() +
  labs(x = "Percent Below LOD", y = "norm(X-L-S) / norm(X)",
       title = "Relative Error in Low-Rank Solution Left Singular Vectors")
```
...
sim_all %>% 
  ...
  #filter(Compare != "Sparse Solution") %>% 
  ggplot(aes(x = Value, y = ..density..)) + 
  geom_histogram(aes(fill = Compare, color = Compare)) + 
  theme_bw() +
  facet_wrap(.~Percent, scales = "free") +
  labs(x = "Solution Value", 
       y = "Density",
