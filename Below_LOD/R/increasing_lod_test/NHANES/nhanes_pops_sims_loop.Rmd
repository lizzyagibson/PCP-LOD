...
sim_all %>% select(seed, grep("S_lod", colnames(.))) %>%
  ...
  mutate(sum = map(sparse, function(x) sum(x)/(1000*8))) %>% 
...
sim_svd <- sim_svd %>% colnames()
  mutate(d_norm = pmap(list(d_lod0_old, d_lod10_old, d_lod20_old, d_lod30_old, d_lod40_old, d_lod50_old, "LOD PCP"), make_dnorm),
         ...
         d_norm_sqrt2 = pmap(list(d_lod0_sqrt2_old, d_lod10_sqrt2_old, d_lod20_sqrt2_old, d_lod30_sqrt2_old, d_lod40_sqrt2_old,
