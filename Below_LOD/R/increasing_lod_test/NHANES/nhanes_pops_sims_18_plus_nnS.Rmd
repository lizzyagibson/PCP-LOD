...
plotall %>% 
  ...
  labs(x = "Solution Value", 
       y = "Density",
       title = "Values <LOD (excluding sparse)") +
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 90, hjust = 1))
...
plotall_nns %>% 
  drop_na(.) %>% 
  filter(Compare == "Sparse Solution") %>% 
  summarise(max_sparse = max(Value),
            min_sparse = min(Value))
```

Ok, this actually makes sense because it only includes values <LOD to begin with. Values <LOD all have zeros in the solution sparse matrix, so model prediction = low rank solution, so you can't see the low rank solution values.