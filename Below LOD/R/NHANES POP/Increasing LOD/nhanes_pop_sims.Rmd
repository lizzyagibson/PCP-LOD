...
cormat <- round(cor(pops, method = c("spearman")),2)
...
ggplot(data = melted_cormat, aes(x = Var1, y = Var2)) +
  ...
  theme_grey() + labs(x = "", y = "", title = "NHANES correlations") +
...
```{r}
cormat2 <- round(cor(sim_all, method = c("spearman")),2)
...
ggplot(data = melted_cormat2, aes(x = Var1, y = Var2)) +
  ...
  scale_fill_gradient2(low = "#00BFC4", mid = "white", high = "#F8766D",
                       midpoint = 0,
                       na.value = "transparent", limits = c(-1, 1)) +
  theme_grey() + labs(x = "", y = "", title = "Simulated correlations") +
  theme(axis.text.x = element_blank(), 
        legend.position = "bottom")
```
...
((rel_error_0  + ylim(0, 0.35)) + 
 (rel_error_s2 + ylim(0, 0.35) + ylab("")))
```
...
V_lod40_0[,2] <- -V_lod40_0[,2]
V_lod40_0[,3] <- -V_lod40_0[,3]
V_lod40_0[,4] <- -V_lod40_0[,4]
V_lod50_0[,2] <- -V_lod50_0[,2]
...
V_lod30_sqrt2[,4] <- -V_lod30_sqrt2[,4]
...
# Hack -- Flip
nn_u_lod10[,4] <- -nn_u_lod10[,4]
nn_u_lod20[,4] <- -nn_u_lod20[,4]
...
u_lod40_0[,2] <- -u_lod40_0[,2]
u_lod40_0[,3] <- -u_lod40_0[,3]
u_lod40_0[,4] <- -u_lod40_0[,4]
u_lod50_0[,2] <- -u_lod50_0[,2]
...
u_lod30_sqrt2[,4] <- -u_lod30_sqrt2[,4]
