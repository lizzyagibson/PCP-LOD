...
sim_all <- sim_all %>% 
  mutate(results_0_0  = pmap(list(sims, lambda_mix, mu_mix), function(x,y,z) pcp_lod(x,y,z, 0)),
         results_10_0 = pmap(list(mix_data_lod_10_0, lambda_mix, mu_mix), 
                           function(x,y,z) pcp_lod(x,y,z, 0)),
...
sim_all <- sim_all %>% 
  mutate(results_0_2  = pmap(list(sims, lambda_mix, mu_mix), function(x,y,z) pcp_lod(x,y,z, 0)),
...
sim_all <- sim_all %>% 
  mutate(results_0_sqrt2  = pmap(list(sims, lambda_mix, mu_mix), function(x,y,z) pcp_lod(x,y,z, 0)),
...
```{r, tables1, cache=TRUE, cache.lazy=FALSE}
sim_all %>% select(seed, grep("L_lod", colnames(.))) %>%
  gather(key = Label, value = Matrix, -seed) %>% 
  mutate(rank = map(Matrix, rankMatrix)) %>%
  select(-Matrix) %>% 
  unnest(cols = rank) %>%
  group_by(Label) %>% 
  summarize(min = min(rank),
            med = median(rank),
            max = max(rank)) %>% 
  filter(!(grepl("0_0", Label) | grepl("0_2", Label))) %>% kable(caption = "Rank of low rank")

sim_all %>% select(seed, grep("S_lod", colnames(.))) %>%
  gather(key = Label, value = Matrix, -seed) %>% 
  mutate(sparse = map(Matrix, function(x) ifelse(x > 0.0001, 1, 0))) %>%
  mutate(sum = map(sparse, function(x) sum(x)/(1000*18))) %>% 
  select(-Matrix, -sparse) %>% 
  unnest(cols = sum) %>%
  group_by(Label) %>% 
  summarize(min = min(sum),
            med = median(sum),
            max = max(sum)) %>% 
  filter(!(grepl("0_0", Label) | grepl("0_2", Label))) %>% kable(caption = "Proportion non-sparse")
```
...
```{r}
sim_all %>% 
  select(seed, grep("L_lod", colnames(.))) %>%
  gather(key = Label, value = DF, -seed) %>% 
  mutate(DF = map(DF, as_tibble)) %>% 
  unnest(cols=DF) %>% 
  select(seed, Label, V1) %>%
  mutate(percent = case_when(grepl("10", Label) ~ "10%",
                           grepl("20", Label) ~ "20%",
                           grepl("30", Label) ~ "30%",
                           grepl("40", Label) ~ "40%", 
                           grepl("50", Label) ~ "50%", 
                           TRUE ~ "0%"),
         model = case_when(grepl("0_2", Label) ~ "LOD/2",
                           grepl("0_0", Label) ~ "LOD/0",
                           grepl("0_nnS", Label) ~ "PCP-LOD nns",
                           grepl("sqrt", Label) ~ "LOD/sqrt(2)", TRUE ~ "PCP-LOD")) %>% 
  ggplot(aes(x = V1, group = Label, color = model, fill = model)) +
  geom_density(alpha = 0.25) +
  facet_wrap(.~percent) + theme_bw() +
  theme(legend.position = "bottom") + labs(y = "Density")

sim_all %>% 
  select(seed, L_lod50_nnS) %>% 
  filter(seed == 1) %>%
  mutate(L_lod50_nnS = map(L_lod50_nnS, as_tibble)) %>% 
  unnest(cols=L_lod50_nnS) %>% 
  gather(key = Chemical, value = Value, -seed) %>% 
  ggplot(aes(x = Value)) +
  geom_histogram() +
  facet_wrap(.~Chemical)

sim_all %>% 
  select(seed, L_lod50_sqrt2) %>% 
  filter(seed == 1) %>%
  mutate(L_lod50_sqrt2 = map(L_lod50_sqrt2, as_tibble)) %>% 
  unnest(cols=L_lod50_sqrt2) %>% 
  gather(key = Chemical, value = Value, -seed) %>% 
  ggplot(aes(x = Value)) +
  geom_histogram() +
  facet_wrap(.~Chemical)
...
```{r, pred_fig, cache=TRUE, fig.align = "center"}
out_f <- sim_f %>% 
  select(seed, F_norm) %>% 
...
```{r, pred_fig95, cache=TRUE, fig.align = "center"}
out_f %>% 
  group_by(Method, percent_blod) %>% 
  summarize(q25 = quantile(norm, probs = .025),
    median = median(norm),
    q75 = quantile(norm, probs = .975)) %>% 
  ggplot(aes(x = percent_blod, y = median, group = Method, ymin = q25, ymax = q75, color = Method)) + 
    geom_point() + 
  geom_errorbar(width=0.1) +
  geom_path(aes(color = Method)) + theme_bw() +
  labs(x = "Percent Below LOD", y = "norm(X-L-S) / norm(X)",
       title = "Relative Prediction Error")
```

```{r, pred_fig95sub, cache=TRUE, fig.align = "center"}
out_f %>% 
  filter(!(Method %in% c("LOD PCP", "PCP w/ 0", "PCP w/ LOD/2"))) %>% 
  mutate(Method = ifelse(Method == "LOD PCP nnS", "PCP-LOD", Method)) %>% 
  group_by(Method, percent_blod) %>% 
  summarize(q25 = quantile(norm, probs = .025),
    median = median(norm),
    q75 = quantile(norm, probs = .975)) %>% 
  ...
  geom_errorbar(width=0.1) +
  geom_path(aes(color = Method)) + theme_bw() +
  labs(x = "Percent Below LOD", y = "Relative Prediction Error")
```
...
```{r, ls_fig, cache=TRUE, fig.align = "center"}
out_ls <- sim_ls %>% 
  select(seed, L_diff) %>% 
...
```{r, ls_fi95, cache=TRUE, fig.align="center"}
out_ls %>% 
  group_by(Method, percent_blod, Matrix) %>% 
  summarize(q25 = quantile(norm, probs = .025),
            median = median(norm),
            q75 = quantile(norm, probs = .975)) %>% 
  ggplot(aes(x = percent_blod, y = median, group = interaction(Method,Matrix), 
             ymin = q25, ymax = q75, color = Method)) + 
    geom_point() + 
  geom_errorbar(width=0.1) +
  geom_path(aes(linetype = Matrix, color = Method)) + theme_bw() +
  labs(x = "Percent Below LOD", y = "norm(difference) / norm(lod0))",
       title = "Relative Error in Solution Matrices")
```

```{r,ls_fig95sub2, cache=TRUE, fig.align = "center"}
out_ls %>% 
  filter(!(Method %in% c("LOD PCP", "PCP w/ 0", "PCP w/ LOD/2"))) %>% 
  mutate(Method = ifelse(Method == "LOD PCP nnS", "PCP-LOD", Method)) %>% 
  group_by(Method, percent_blod, Matrix) %>% 
  ...
  geom_errorbar(width=0.1) +
  geom_path(aes(linetype = Matrix, color = Method)) + theme_bw() +
  labs(x = "Percent Below LOD", y = "Relative Difference in Solution Matrices")
```
  ...
  mutate(above_diff = pmap(list(sims, l_above_blod_0, s_above_blod_0, 
         ...
         above_diff_2 = pmap(list(sims, l_above_blod_0_2, s_above_blod_0_2, 
          ...
          above_diff_sqrt2 = pmap(list(sims, l_above_blod_0_sqrt2, s_above_blod_0_sqrt2, 
                                       ...
                                       paste0("PCP w/ LOD/", intToUtf8(8730),2)), 
                                  function (a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s)
                             make_above_diff(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s)),
  ...
  mutate(less_diff = pmap(list(blod_0, l_blod_0, s_blod_0, 
         ...
         less_diff_sqrt2 = pmap(list(blod_0, l_blod_0_sqrt2, s_blod_0_sqrt2, 
                                     ...
                                     blod_50, l_blod_50_sqrt2, s_blod_50_sqrt2, 
                                      paste0("PCP w/ LOD/", intToUtf8(8730),2)), make_below_diff),
...
```{r, ab_fig, cache=TRUE, cache.lazy=FALSE, fig.align = "center"}
out_ab <- sim_ab %>% 
...
```{r, ab_fi95, cache=TRUE, cache.lazy=FALSE, fig.align="center"}
out_ab %>% 
  group_by(Method, percent_blod, Values) %>% 
  summarize(q25 = quantile(norm, probs = .025),
            median = median(norm),
            q75 = quantile(norm, probs = .975)) %>% 
  mutate(median = ifelse(Values == "> LOD" & Method == "LOD PCP", median + 0.005, median),
         median = ifelse(Values == "> LOD" & Method == "PCP w/ 0", median - 0.005, median)) %>% 
  ggplot(aes(x = percent_blod, y = median, group = interaction(Values, Method), color = Method, ymin = q25, ymax = q75)) + 
    geom_point() + 
  geom_errorbar(width=0.1) +
  geom_path(aes(linetype = Values)) + theme_bw() +
  labs(x = "Percent Below LOD", 
       y = "(norm(X-L-S) / norm(X)",
       title = "Relative Error in Values < LOD & > LOD")
```

```{r,ls_fig95sub, cache=TRUE, fig.align = "center"}
out_ab %>% 
  filter(!(Method %in% c("LOD PCP", "PCP w/ 0", "PCP w/ LOD/2"))) %>% 
  mutate(Method = ifelse(Method == "LOD PCP nnS", "PCP-LOD", Method)) %>% 
  group_by(Method, percent_blod, Values) %>% 
  ...
  geom_path(aes(linetype = Values)) + theme_bw() +
  labs(x = "Percent Below LOD", 
       y = "Relative Prediction Error in Values < LOD & > LOD")
```
...
sim_svd <- sim_svd %>% 
        mutate(v_norm = pmap(list(v_lod0, v_lod10, v_lod20, v_lod30, v_lod40, v_lod50, "LOD PCP"), 
         ...
         v_norm_sqrt2 = pmap(list(v_lod0_sqrt2, v_lod10_sqrt2, v_lod20_sqrt2, v_lod30_sqrt2, v_lod40_sqrt2,
                                  v_lod50_sqrt2, paste0("PCP w/ LOD/", intToUtf8(8730),2)), 
            function (mix, L10, L20, L30, L40, L50, Name) make_Vnorm(mix, L10, L20, L30, L40, L50, Name)),
...
```{r, v_fig, cache=TRUE, cache.lazy=FALSE, fig.align = "center"}
...
```{r, v_fig95, message=FALSE, cache=TRUE, cache.lazy=FALSE, fig.align="center"}
...
out_v %>% 
  ...
  summarize(q25 = quantile(norm, probs = .025),
            median = median(norm),
            q75 = quantile(norm, probs = .975)) %>% 
  ggplot(aes(x = percent_blod, y = median, ymin = q25, ymax = q75, group = Method, color = Method)) + 
    geom_point() + 
  geom_errorbar(width=0.1) +
  geom_path(aes(color = Method)) + theme_bw() +
  labs(x = "Percent Below LOD", y = "norm(X-L-S) / norm(X)",
...
```{r,v_fig95sub, cache=TRUE, fig.align = "center"}
...
out_v %>% 
  ...
  geom_errorbar(width=0.1) +
  geom_path(aes(color = Method)) + theme_bw() +
  labs(x = "Percent Below LOD", y = "Relative Difference in Low-Rank Solution Right Singular Vectors")
```

```{r, v_figdense, message=FALSE, cache=TRUE, cache.lazy=FALSE, fig.align="center"}

out_v %>% 
  filter(percent_blod != "0%") %>% 
  ggplot(aes(x = norm)) + 
  geom_histogram(aes(fill = Method, color = Method), alpha = 0.25) + 
  theme_bw() +
  facet_wrap(.~percent_blod, scales = "free") +
  labs(x = "Solution Value", 
       y = "Count",
       title = "Distribution of Relative Norms of Low-Rank Solution Right Singular Vectors") +
...
sim_svd <- sim_svd %>% 
  mutate(u_norm = pmap(list(u_lod0, u_lod10, u_lod20, u_lod30, u_lod40, u_lod50, "LOD PCP"), make_unorm),
         ...
         u_norm_sqrt2 = pmap(list(u_lod0_sqrt2, u_lod10_sqrt2, u_lod20_sqrt2, u_lod30_sqrt2, u_lod40_sqrt2,
                                  u_lod50_sqrt2, paste0("PCP w/ LOD/", intToUtf8(8730),2)), make_unorm),
...
```{r, u_fig, cache=TRUE, cache.lazy=FALSE, fig.align = "center"}
...
```{r, u_fig95, cache=TRUE, cache.lazy=FALSE, fig.align="center"}
...
```{r,u_fig95sub, cache=TRUE, fig.align = "center"}
...
out_u %>% 
  ...
  geom_errorbar(width=0.1) +
  geom_path(aes(color = Method)) + theme_bw() +
  labs(x = "Percent Below LOD", y = "Relative Difference in Low-Rank Solution Left Singular Vectors")
```

```{r, u_figdense, message=FALSE, cache=TRUE, cache.lazy=FALSE, fig.align="center"}
...
out_u %>% 
  ...
  labs(x = "Solution Value", 
       y = "Count",
       title = "Distribution of Relative Norms of Low-Rank Solution Left Singular Vectors") +
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 90, hjust = 1))
```
...
sim_svd <- sim_svd %>% 
  mutate(d_norm = pmap(list(d_lod0_old, d_lod10_old, d_lod20_old, d_lod30_old, 
                            d_lod40_old, d_lod50_old, "LOD PCP"), make_dnorm),
         d_norm_2 = pmap(list(d_lod0_2_old, d_lod10_2_old, d_lod20_2_old, d_lod30_2_old,
                              d_lod40_2_old, d_lod50_2_old, "PCP w/ LOD/2"), make_dnorm),
         d_norm_sqrt2 = pmap(list(d_lod0_sqrt2_old, d_lod10_sqrt2_old, d_lod20_sqrt2_old, 
                                  d_lod30_sqrt2_old, d_lod40_sqrt2_old,
                                  d_lod50_sqrt2_old, paste0("PCP w/ LOD/", intToUtf8(8730),2)), make_dnorm),
         d_norm_nnS = pmap(list(d_lod0_nnS_old, d_lod10_nnS_old, d_lod20_nnS_old, d_lod30_nnS_old, d_lod40_nnS_old,
                                d_lod50_nnS_old, "LOD PCP nnS"), make_dnorm),
         d_norm_0 = pmap(list(d_lod0_0_old, d_lod10_0_old, d_lod20_0_old, d_lod30_0_old, d_lod40_0_old,
                              d_lod50_0_old, "PCP w/ 0"), make_dnorm))
```

* Reference is singular value vector of SVD of low rank solution matix with 0\% \<LOD

```{r, d_fig2, fig.align = "center", cache=TRUE, cache.lazy=FALSE}
out_d <- sim_svd %>%
  select(seed, d_norm) %>%
...
```{r, d_fig95, cache=TRUE, cache.lazy=FALSE, fig.align="center"}
out_d %>%
  group_by(Method, percent_blod) %>%
  summarize(q25 = quantile(norm, probs = .025),
            median = median(norm),
...
```{r, d_fig95sub, cache=TRUE, fig.align = "center"}
out_d %>%
  filter(!(Method %in% c("LOD PCP", "PCP w/ 0", "PCP w/ LOD/2"))) %>%
  mutate(Method = ifelse(Method == "LOD PCP nnS", "PCP-LOD", Method)) %>%
  group_by(Method, percent_blod) %>%
  ...
  geom_errorbar(width=0.1) +
  geom_path(aes(color = Method)) + theme_bw() +
  labs(x = "Percent Below LOD", y = "Relative Difference in Low-Rank Solution Singular Values")
```

```{r, d_figdense, message=FALSE, cache=TRUE, cache.lazy=FALSE, fig.align="center"}
out_d %>%
  filter(percent_blod != "0%") %>%
  ggplot(aes(x = norm)) +
  geom_histogram(aes(fill = Method, color = Method), alpha = 0.25) +
  ...
  labs(x = "Solution Value",
       y = "Count",
       title = "Distribution of Relative Norms of Low-Rank Solution Singular Values") +
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 90, hjust = 1))
```
...
```{r, below_lod, include=FALSE, cache = TRUE, cache.lazy=FALSE}
create_below_NA <- function (sim_neg1, solution) {
  tf_lod <- sim_neg1 %>% 
  as_tibble() %>% 
...
sim_all <- sim_all %>% 
  mutate(blod_na_0 = map2(sims, sims, create_below_NA),
         blod_na_10 = map2(mix_data_lod_10, sims, create_below_NA),
         blod_na_20 = map2(mix_data_lod_20, sims, create_below_NA),
         blod_na_30 = map2(mix_data_lod_30, sims, create_below_NA),
         blod_na_40 = map2(mix_data_lod_40, sims, create_below_NA),
         blod_na_50 = map2(mix_data_lod_50, sims, create_below_NA))

sim_all <- sim_all %>% 
  mutate(l_blod_na_0 = map2(as.matrix(sims), L_lod0, create_below_NA),
         s_blod_na_0 = map2(sims, S_lod0, create_below_NA),
         l_blod_na_10 = map2(mix_data_lod_10, L_lod10, create_below_NA),
         s_blod_na_10 = map2(mix_data_lod_10, S_lod10, create_below_NA),
         l_blod_na_20 = map2(mix_data_lod_20, L_lod20, create_below_NA),
         s_blod_na_20 = map2(mix_data_lod_20, S_lod20, create_below_NA),
         l_blod_na_30 = map2(mix_data_lod_30, L_lod30, create_below_NA),
         s_blod_na_30 = map2(mix_data_lod_30, S_lod30, create_below_NA),
         l_blod_na_40 = map2(mix_data_lod_40, L_lod40, create_below_NA),
         s_blod_na_40 = map2(mix_data_lod_40, S_lod40, create_below_NA),
         l_blod_na_50 = map2(mix_data_lod_50, L_lod50, create_below_NA),
         s_blod_na_50 = map2(mix_data_lod_50, S_lod50, create_below_NA),
       l_blod_na_0_sqrt2 = map2(sims, L_lod0_sqrt2, create_below_NA),
       s_blod_na_0_sqrt2 = map2(sims, S_lod0_sqrt2, create_below_NA),
       l_blod_na_10_sqrt2 = map2(mix_data_lod_10, L_lod10_sqrt2, create_below_NA),
       s_blod_na_10_sqrt2 = map2(mix_data_lod_10, S_lod10_sqrt2, create_below_NA),
       l_blod_na_20_sqrt2 = map2(mix_data_lod_20, L_lod20_sqrt2, create_below_NA),
       s_blod_na_20_sqrt2 = map2(mix_data_lod_20, S_lod20_sqrt2, create_below_NA),
       l_blod_na_30_sqrt2 = map2(mix_data_lod_30, L_lod30_sqrt2, create_below_NA),
       s_blod_na_30_sqrt2 = map2(mix_data_lod_30, S_lod30_sqrt2, create_below_NA),
       l_blod_na_40_sqrt2 = map2(mix_data_lod_40, L_lod40_sqrt2, create_below_NA),
       s_blod_na_40_sqrt2 = map2(mix_data_lod_40, S_lod40_sqrt2, create_below_NA),
       l_blod_na_50_sqrt2 = map2(mix_data_lod_50, L_lod50_sqrt2, create_below_NA),
       s_blod_na_50_sqrt2 = map2(mix_data_lod_50, S_lod50_sqrt2, create_below_NA),
       l_blod_na_0_nnS = map2(sims, L_lod0_nnS, create_below_NA),
       s_blod_na_0_nnS = map2(sims, S_lod0_nnS, create_below_NA),
       l_blod_na_10_nnS = map2(mix_data_lod_10, L_lod10_nnS, create_below_NA),
       s_blod_na_10_nnS = map2(mix_data_lod_10, S_lod10_nnS, create_below_NA),
       l_blod_na_20_nnS = map2(mix_data_lod_20, L_lod20_nnS, create_below_NA),
       s_blod_na_20_nnS = map2(mix_data_lod_20, S_lod20_nnS, create_below_NA),
       l_blod_na_30_nnS = map2(mix_data_lod_30, L_lod30_nnS, create_below_NA),
       s_blod_na_30_nnS = map2(mix_data_lod_30, S_lod30_nnS, create_below_NA),
       l_blod_na_40_nnS = map2(mix_data_lod_40, L_lod40_nnS, create_below_NA),
       s_blod_na_40_nnS = map2(mix_data_lod_40, S_lod40_nnS, create_below_NA),
       l_blod_na_50_nnS = map2(mix_data_lod_50, L_lod50_nnS, create_below_NA),
       s_blod_na_50_nnS = map2(mix_data_lod_50, S_lod50_nnS, create_below_NA))
```

```{r, lod, include=FALSE, cache = TRUE, cache.lazy=FALSE}
sim_all <- sim_all %>% 
  mutate(LOD10 = map(delta10, function (x) {x %>% as.matrix() %>% t() %>% as_tibble() %>% 
      rename_all(., list(~str_sub(., start = 1, end = -5))) %>% gather(POP, LOD) %>% mutate(Percent = "10%")}),
      LOD20 = map(delta20, function (x) {x %>% as.matrix() %>% t() %>% as_tibble() %>% 
      rename_all(., list(~str_sub(., start = 1, end = -5))) %>% gather(POP, LOD) %>% mutate(Percent = "20%")}),
      LOD30 = map(delta30, function (x) {x %>% as.matrix() %>% t() %>% as_tibble() %>% 
      rename_all(., list(~str_sub(., start = 1, end = -5))) %>% gather(POP, LOD) %>% mutate(Percent = "30%")}),
      LOD40 = map(delta40, function (x) {x %>% as.matrix() %>% t() %>% as_tibble() %>% 
      rename_all(., list(~str_sub(., start = 1, end = -5))) %>% gather(POP, LOD) %>% mutate(Percent = "40%")}),
      LOD50 = map(delta50, function (x) {x %>% as.matrix() %>% t() %>% as_tibble() %>% 
      rename_all(., list(~str_sub(., start = 1, end = -5))) %>% gather(POP, LOD) %>% mutate(Percent = "50%")}))

sim_all <- sim_all %>% mutate(LODall = pmap(list(LOD10, LOD20, LOD30, LOD40, LOD50),
                                function (LOD10, LOD20, LOD30, LOD40, LOD50) 
                                  {rbind(LOD10, LOD20, LOD30, LOD40, LOD50)}))
```

```{r, below_lod2, include=FALSE, cache = TRUE, cache.lazy=FALSE}
sim_all <- sim_all %>% 
  mutate(plot_10 = map(blod_na_10, function (x) 
    {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% mutate(Percent = "10%")}),
    plot_20 = map(blod_na_20, function (x) 
    {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% mutate(Percent = "20%")}),
    plot_30 = map(blod_na_30, function (x) 
    {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% mutate(Percent = "30%")}),
    plot_40 = map(blod_na_40, function (x) 
    {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% mutate(Percent = "40%")}),
    plot_50 = map(blod_na_50, function (x) 
    {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% mutate(Percent = "50%")}))

sim_all <- sim_all %>% 
  mutate(org = pmap(list(plot_10, plot_20, plot_30, plot_40, plot_50, LODall),
                    function (plot_10, plot_20, plot_30, plot_40, plot_50, LODall)
                      {rbind(plot_10, plot_20, plot_30, plot_40, plot_50) %>%  
  mutate(Compare = "Original") %>% 
  left_join(., LODall, by = c("POP", "Percent"))}))
```

```{r, below_lod3, include=FALSE, cache = TRUE, cache.lazy=FALSE}
sim_all <- sim_all %>% 
  mutate(plotl_10 = map2(l_blod_na_10, LOD10, function(x,y) 
    {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Low Rank Solution", Percent = "10%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
    plots_10 = map2(s_blod_na_10, LOD10, function(x,y) 
      {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
        gather(key = POP, value = Value, -rowid) %>% 
         mutate(Compare = "Sparse Solution", Percent = "10%") %>% 
        left_join(., y, by = c("POP", "Percent"))}),
    plotall_10 = pmap(list(l_blod_na_10, s_blod_na_10, LOD10), function(x,y,z) 
      {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
        gather(key = POP, value = Value, -rowid) %>% 
        mutate(Compare = "Model Prediction", Percent = "10%") %>%
        left_join(., z, by = c("POP", "Percent"))}),
          plotl_20 = map2(l_blod_na_20, LOD20, function(x,y) 
      {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
          gather(key = POP, value = Value, -rowid) %>% 
          mutate(Compare = "Low Rank Solution", Percent = "20%") %>% 
          left_join(., y, by = c("POP", "Percent"))}),
      plots_20 = map2(s_blod_na_20, LOD20, function(x,y) 
      {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
          gather(key = POP, value = Value, -rowid) %>% 
          mutate(Compare = "Sparse Solution", Percent = "20%") %>% 
          left_join(., y, by = c("POP", "Percent"))}),
      plotall_20 = pmap(list(l_blod_na_20, s_blod_na_20, LOD20), function(x,y,z) 
      {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
          gather(key = POP, value = Value, -rowid) %>% 
          mutate(Compare = "Model Prediction", Percent = "20%") %>%
          left_join(., z, by = c("POP", "Percent"))}),
          plotl_30 = map2(l_blod_na_30, LOD30, function(x,y) 
      {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
          gather(key = POP, value = Value, -rowid) %>% 
          mutate(Compare = "Low Rank Solution", Percent = "30%") %>% 
          left_join(., y, by = c("POP", "Percent"))}),
      plots_30 = map2(s_blod_na_30, LOD30, function(x,y) 
      {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
          gather(key = POP, value = Value, -rowid) %>% 
          mutate(Compare = "Sparse Solution", Percent = "30%") %>% 
          left_join(., y, by = c("POP", "Percent"))}),
      plotall_30 = pmap(list(l_blod_na_30, s_blod_na_30, LOD30), function(x,y,z) 
      {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
          gather(key = POP, value = Value, -rowid) %>% 
          mutate(Compare = "Model Prediction", Percent = "30%") %>%
          left_join(., z, by = c("POP", "Percent"))}),
          plotl_40 = map2(l_blod_na_40, LOD40, function(x,y) 
      {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
          gather(key = POP, value = Value, -rowid) %>% 
          mutate(Compare = "Low Rank Solution", Percent = "40%") %>% 
          left_join(., y, by = c("POP", "Percent"))}),
      plots_40 = map2(s_blod_na_40, LOD40, function(x,y) 
      {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
          gather(key = POP, value = Value, -rowid) %>% 
          mutate(Compare = "Sparse Solution", Percent = "40%") %>% 
          left_join(., y, by = c("POP", "Percent"))}),
      plotall_40 = pmap(list(l_blod_na_40, s_blod_na_40, LOD40), function(x,y,z) 
      {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
          gather(key = POP, value = Value, -rowid) %>% 
          mutate(Compare = "Model Prediction", Percent = "40%") %>%
          left_join(., z, by = c("POP", "Percent"))}),
                plotl_50 = map2(l_blod_na_50, LOD50, function(x,y) 
      {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
          gather(key = POP, value = Value, -rowid) %>% 
          mutate(Compare = "Low Rank Solution", Percent = "50%") %>% 
          left_join(., y, by = c("POP", "Percent"))}),
      plots_50 = map2(s_blod_na_50, LOD50, function(x,y) 
      {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
          gather(key = POP, value = Value, -rowid) %>% 
          mutate(Compare = "Sparse Solution", Percent = "50%") %>% 
          left_join(., y, by = c("POP", "Percent"))}),
      plotall_50 = pmap(list(l_blod_na_50, s_blod_na_50, LOD50), function(x,y,z) 
      {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
          gather(key = POP, value = Value, -rowid) %>% 
          mutate(Compare = "Model Prediction", Percent = "50%") %>%
          left_join(., z, by = c("POP", "Percent"))}))

sim_all <- sim_all %>% 
  mutate(plotall = pmap(list(plotl_10,plots_10,plotall_10,plotl_20,plots_20,plotall_20,
                             plotl_30,plots_30,plotall_30,plotl_40,plots_40,plotall_40,
                             plotl_50,plots_50,plotall_50),
                        function(plotl_10,plots_10,plotall_10,plotl_20,plots_20,plotall_20,
                                plotl_30,plots_30,plotall_30,plotl_40,plots_40,plotall_40,
                                plotl_50,plots_50,plotall_50) 
                          {rbind(plotl_10,plots_10,plotall_10,plotl_20,plots_20,plotall_20,
                              plotl_30,plots_30,plotall_30,plotl_40,plots_40,plotall_40,
                              plotl_50,plots_50,plotall_50)}))
```

```{r, below_lod4, include=FALSE, cache=TRUE, cache.lazy=FALSE}
sim_all <- sim_all %>% 
  mutate(plotl_10_sqrt2 = map2(l_blod_na_10_sqrt2, LOD10, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Low Rank Solution", Percent = "10%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plots_10_sqrt2 = map2(s_blod_na_10_sqrt2, LOD10, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Sparse Solution", Percent = "10%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plotall_10_sqrt2 = pmap(list(l_blod_na_10_sqrt2, s_blod_na_10_sqrt2, LOD10), function(x,y,z) 
  {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Model Prediction", Percent = "10%") %>%
      left_join(., z, by = c("POP", "Percent"))}),
  plotl_20_sqrt2 = map2(l_blod_na_20_sqrt2, LOD20, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Low Rank Solution", Percent = "20%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plots_20_sqrt2 = map2(s_blod_na_20_sqrt2, LOD20, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Sparse Solution", Percent = "20%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plotall_20_sqrt2 = pmap(list(l_blod_na_20_sqrt2, s_blod_na_20_sqrt2, LOD20), function(x,y,z) 
  {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Model Prediction", Percent = "20%") %>%
      left_join(., z, by = c("POP", "Percent"))}),
  plotl_30_sqrt2 = map2(l_blod_na_30_sqrt2, LOD30, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Low Rank Solution", Percent = "30%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plots_30_sqrt2 = map2(s_blod_na_30_sqrt2, LOD30, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Sparse Solution", Percent = "30%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plotall_30_sqrt2 = pmap(list(l_blod_na_30_sqrt2, s_blod_na_30_sqrt2, LOD30), function(x,y,z) 
  {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Model Prediction", Percent = "30%") %>%
      left_join(., z, by = c("POP", "Percent"))}),
  plotl_40_sqrt2 = map2(l_blod_na_40_sqrt2, LOD40, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Low Rank Solution", Percent = "40%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plots_40_sqrt2 = map2(s_blod_na_40_sqrt2, LOD40, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Sparse Solution", Percent = "40%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plotall_40_sqrt2 = pmap(list(l_blod_na_40_sqrt2, s_blod_na_40_sqrt2, LOD40), function(x,y,z) 
  {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Model Prediction", Percent = "40%") %>%
      left_join(., z, by = c("POP", "Percent"))}),
  plotl_50_sqrt2 = map2(l_blod_na_50_sqrt2, LOD50, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Low Rank Solution", Percent = "50%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plots_50_sqrt2 = map2(s_blod_na_50_sqrt2, LOD50, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Sparse Solution", Percent = "50%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plotall_50_sqrt2 = pmap(list(l_blod_na_50_sqrt2, s_blod_na_50_sqrt2, LOD50), function(x,y,z) 
  {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Model Prediction", Percent = "50%") %>%
      left_join(., z, by = c("POP", "Percent"))}))
...
sim_all <- sim_all %>% 
  mutate(plotall_sqrt2 = pmap(list(plotl_10_sqrt2,plots_10_sqrt2,plotall_10_sqrt2,plotl_20_sqrt2,plots_20_sqrt2,plotall_20_sqrt2,
                             plotl_30_sqrt2,plots_30_sqrt2,plotall_30_sqrt2,plotl_40_sqrt2,plots_40_sqrt2,plotall_40_sqrt2,
                             plotl_50_sqrt2,plots_50_sqrt2,plotall_50_sqrt2),
                        function(plotl_10_sqrt2,plots_10_sqrt2,plotall_10_sqrt2,plotl_20_sqrt2,plots_20_sqrt2,plotall_20_sqrt2,
                                 plotl_30_sqrt2,plots_30_sqrt2,plotall_30_sqrt2,plotl_40_sqrt2,plots_40_sqrt2,plotall_40_sqrt2,
                                 plotl_50_sqrt2,plots_50_sqrt2,plotall_50_sqrt2) 
                        {rbind(plotl_10_sqrt2,plots_10_sqrt2,plotall_10_sqrt2,plotl_20_sqrt2,plots_20_sqrt2,plotall_20_sqrt2,
                               plotl_30_sqrt2,plots_30_sqrt2,plotall_30_sqrt2,plotl_40_sqrt2,plots_40_sqrt2,plotall_40_sqrt2,
                               plotl_50_sqrt2,plots_50_sqrt2,plotall_50_sqrt2)}))
```

```{r, below_lod5, include=FALSE, cache=TRUE, cache.lazy=FALSE}
sim_all <- sim_all %>% 
  mutate(plotl_10_nnS = map2(l_blod_na_10_nnS, LOD10, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Low Rank Solution", Percent = "10%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plots_10_nnS = map2(s_blod_na_10_nnS, LOD10, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Sparse Solution", Percent = "10%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plotall_10_nnS = pmap(list(l_blod_na_10_nnS, s_blod_na_10_nnS, LOD10), function(x,y,z) 
  {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Model Prediction", Percent = "10%") %>%
      left_join(., z, by = c("POP", "Percent"))}),
  plotl_20_nnS = map2(l_blod_na_20_nnS, LOD20, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Low Rank Solution", Percent = "20%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plots_20_nnS = map2(s_blod_na_20_nnS, LOD20, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Sparse Solution", Percent = "20%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plotall_20_nnS = pmap(list(l_blod_na_20_nnS, s_blod_na_20_nnS, LOD20), function(x,y,z) 
  {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Model Prediction", Percent = "20%") %>%
      left_join(., z, by = c("POP", "Percent"))}),
  plotl_30_nnS = map2(l_blod_na_30_nnS, LOD30, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Low Rank Solution", Percent = "30%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plots_30_nnS = map2(s_blod_na_30_nnS, LOD30, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Sparse Solution", Percent = "30%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plotall_30_nnS = pmap(list(l_blod_na_30_nnS, s_blod_na_30_nnS, LOD30), function(x,y,z) 
  {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Model Prediction", Percent = "30%") %>%
      left_join(., z, by = c("POP", "Percent"))}),
  plotl_40_nnS = map2(l_blod_na_40_nnS, LOD40, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Low Rank Solution", Percent = "40%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plots_40_nnS = map2(s_blod_na_40_nnS, LOD40, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Sparse Solution", Percent = "40%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plotall_40_nnS = pmap(list(l_blod_na_40_nnS, s_blod_na_40_nnS, LOD40), function(x,y,z) 
  {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Model Prediction", Percent = "40%") %>%
      left_join(., z, by = c("POP", "Percent"))}),
  plotl_50_nnS = map2(l_blod_na_50_nnS, LOD50, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Low Rank Solution", Percent = "50%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plots_50_nnS = map2(s_blod_na_50_nnS, LOD50, function(x,y) 
  {x %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Sparse Solution", Percent = "50%") %>% 
      left_join(., y, by = c("POP", "Percent"))}),
  plotall_50_nnS = pmap(list(l_blod_na_50_nnS, s_blod_na_50_nnS, LOD50), function(x,y,z) 
  {(x + y) %>% as_tibble(.) %>% rowid_to_column(.) %>% 
      gather(key = POP, value = Value, -rowid) %>% 
      mutate(Compare = "Model Prediction", Percent = "50%") %>%
      left_join(., z, by = c("POP", "Percent"))}))
...
sim_all <- sim_all %>% 
  mutate(plotall_nnS = pmap(list(plotl_10_nnS,plots_10_nnS,plotall_10_nnS,plotl_20_nnS,plots_20_nnS,plotall_20_nnS,
                             plotl_30_nnS,plots_30_nnS,plotall_30_nnS,plotl_40_nnS,plots_40_nnS,plotall_40_nnS,
                             plotl_50_nnS,plots_50_nnS,plotall_50_nnS),
                        function(plotl_10_nnS,plots_10_nnS,plotall_10_nnS,plotl_20_nnS,plots_20_nnS,plotall_20_nnS,
                                 plotl_30_nnS,plots_30_nnS,plotall_30_nnS,plotl_40_nnS,plots_40_nnS,plotall_40_nnS,
                                 plotl_50_nnS,plots_50_nnS,plotall_50_nnS) 
                        {rbind(plotl_10_nnS,plots_10_nnS,plotall_10_nnS,plotl_20_nnS,plots_20_nnS,plotall_20_nnS,
                               plotl_30_nnS,plots_30_nnS,plotall_30_nnS,plotl_40_nnS,plots_40_nnS,plotall_40_nnS,
                               plotl_50_nnS,plots_50_nnS,plotall_50_nnS)}))
```
...
```{r, tables, cache=TRUE, cache.lazy=FALSE}
sim_all %>% select(seed, plotall) %>% 
  #filter(seed == 1) %>% 
  unnest(cols = c('plotall')) %>% 
...
sim_all %>% select(seed, plotall_nnS) %>% 
  #filter(seed == 1) %>% 
  unnest(cols = c('plotall_nnS')) %>% 
  filter(Compare == "Low Rank Solution") %>% 
  drop_na(.) %>% 
  mutate(above_lod = ifelse(Value > LOD, 1, 0)) %>%
  group_by(Compare, Percent) %>% 
...
sim_all %>% select(seed, plotall_sqrt2) %>% 
  #filter(seed == 1) %>% 
  unnest(cols = c('plotall_sqrt2')) %>%  
  filter(Compare == "Low Rank Solution") %>% 
  drop_na(.) %>% 
  mutate(above_lod = ifelse(Value > LOD, 1, 0)) %>%
  group_by(Compare, Percent) %>% 
...
sim_all %>% 
  mutate(sparse = pmap(list(plots_10, plots_20, plots_30, plots_40, plots_50),
                       function (plots_10, plots_20, plots_30, plots_40, plots_50)
                         {rbind(plots_10, plots_20, plots_30, plots_40, plots_50)})) %>%
  select(seed, sparse) %>%
  unnest(cols = 'sparse') %>% 
                          drop_na(.) %>% 
                          group_by(Compare, Percent) %>% 
...
sim_all %>% 
  mutate(sparse_nnS = pmap(list(plots_10_nnS, plots_20_nnS, plots_30_nnS, plots_40_nnS, plots_50_nnS),
                       function (plots_10_nnS, plots_20_nnS, plots_30_nnS, plots_40_nnS, plots_50_nnS)
                       {rbind(plots_10_nnS, plots_20_nnS, plots_30_nnS, plots_40_nnS, plots_50_nnS)})) %>%
  select(seed, sparse_nnS) %>%
  unnest(cols = 'sparse_nnS') %>% 
  drop_na(.) %>% 
  group_by(Compare, Percent) %>%
  summarise(Min = min (Value),
            Mean = mean(Value),
...
sim_all %>% 
  mutate(sparse_sqrt2 = pmap(list(plots_10_sqrt2, plots_20_sqrt2, plots_30_sqrt2, plots_40_sqrt2, plots_50_sqrt2),
                       function (plots_10_sqrt2, plots_20_sqrt2, plots_30_sqrt2, plots_40_sqrt2, plots_50_sqrt2)
                       {rbind(plots_10_sqrt2, plots_20_sqrt2, plots_30_sqrt2, plots_40_sqrt2, plots_50_sqrt2)})) %>%
  select(seed, sparse_sqrt2) %>%
  unnest(cols = 'sparse_sqrt2') %>% 
  drop_na(.) %>% 
  group_by(Compare, Percent) %>%
  summarise(Min = min (Value),
            Mean = mean(Value),
...
```{r, fig.align = "center", cache=TRUE, cache.lazy=FALSE}
sim_all %>% 
  select(seed, plotall) %>% 
  unnest(cols = 'plotall') %>% 
  rbind(., (sim_all %>% select(seed, org) %>% unnest(cols = 'org'))) %>% 
  drop_na(.) %>% 
...
```{r, fig.align = "center", cache=TRUE, cache.lazy=FALSE}
sim_all %>% 
  select(seed, plotall_nnS) %>% 
  unnest(cols = 'plotall_nnS') %>% 
  rbind(., (sim_all %>% select(seed, org) %>% unnest(cols = 'org'))) %>% 
  drop_na(.) %>% 
  #filter(Compare != "Original") %>% 
  # with and without Original
...
```{r, fig.align = "center", cache=TRUE, cache.lazy=FALSE}
sim_all %>% 
  select(seed, plotall_sqrt2) %>% 
  unnest(cols = 'plotall_sqrt2') %>% 
  rbind(., (sim_all %>% select(seed, org) %>% unnest(cols = 'org'))) %>% 
  drop_na(.) %>% 
  #filter(Compare != "Original") %>% 
  # with and without Original
...
```{r, message=FALSE, fig.align = "center", cache=TRUE, cache.lazy=FALSE}
sim_all %>% 
  select(seed, plotall) %>% 
  unnest(cols = 'plotall') %>% 
  rbind(., (sim_all %>% select(seed, org) %>% unnest(cols = 'org'))) %>% 
