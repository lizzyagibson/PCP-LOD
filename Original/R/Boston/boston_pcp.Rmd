---
title: "Boston Air Pollution Data"
author: "Lizzy Gibson"
date: "1/23/2020"
output: 
  html_document:
...
mixture_data <- as.data.frame(mixture) %>% 
  as_tibble() %>% select(Al, As, Ba, bc, Br, Ca, Cl,
                         Cr, Cu, Fe, K,  Mn,  Ni,  Pb,  S,  Se,  Si,
                         Ti,  V, Zn) %>% as.matrix()

mix <- mixture_data[complete.cases(mixture_data),]

X <- scale(mix, center = FALSE, scale = TRUE)
...
#svd(X)$d
#svd(prox_nuclear(mix_L, 1)$X)$d
#pmax(abs(0.000020815751050194793783 ) - 1, myzero, na.rm = TRUE)
...
```{r run, cache=TRUE}
mixture_out <- pcp_lod(X, lambda = lambda, mu = 10, Delta = 0)
...
```{r}
svd(mixture_L)$d
svd(mixture_L)$d > 0.01

as_tibble(svd(mixture_L)$v) %>%
  mutate(id = c("Al", "As", "Ba", "bc", "Br", "Ca", "Cl",
                "Cr", "Cu", "Fe", "K",  "Mn",  "Ni",  "Pb",  "S",  "Se",  "Si",
                "Ti",  "V", "Zn")) %>%
  select(id, everything()) %>%
  gather(key = singular_vector, value = magnitude, V1:V10) %>%
  ...
  geom_hline(yintercept = 0, linetype = "dashed",
             color = "red") +
  theme_bw() + labs(x = "", y = "Magnitude", title = "First 10 right singular vectors of V") +
...
```{r run2, cache=TRUE}
mix_out <- pcp_lod(mix, lambda = lambda, mu = 10, Delta = 0)
