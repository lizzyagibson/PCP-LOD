...
for (s in sigma) {
  ...
  read <- readMat(here::here(paste0("HPC_PCP/pcp_38/pcp_all_lod_", d, "_rank_", r, 
                                    "_sigma_", s, ".mat")))
  ...
  pcp_out <- rbind(pcp_out, out_mu) %>% drop_na(iter)
  }
}
}
...
pcp_plot %>% select(reg_mu = rel_error) %>% drop_na() %>% 
  cbind(., pcp_plot_38 %>% select(mu_38 = rel_error) %>% drop_na()) %>% as_tibble()

pcp_plot_38 %>% filter(model == "PCP" & delta != "1" & rank != "1") %>% 
  select(mu_38 = rel_error) %>% summary()
pcp_plot %>% filter(model == "PCP" & delta != "1" & rank != "1") %>% 
  select(reg = rel_error) %>% summary()
...
pcp_plot_38 %>% 
  filter(rank == "4") %>%
  filter(model != "PCA") %>% 
  filter(sigma == "0.6") %>% # high sigma PCP-LOD does better / low sigma PCP-LOD does worse
  filter(delta != "1") %>% 
  ggplot(aes(x = delta, y = rel_error, color = model)) +
  geom_boxplot(notch = TRUE, outlier.size = 0.25) +
  facet_grid(~type) + scale_y_log10() +
