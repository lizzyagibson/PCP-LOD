function [L, S] = root_pcp_rank_r_with_missing (D, lambda, mu, r)
% [L, S] = root_pcp_rank_r_with_missing (D, lambda, mu, r)
%
% Solve the following problem:
% min_{L,S}
%         1_{rank(L) <= r} + lambda * ||S||_1 + mu * ||P_(obs)[L+S-D] ||_F
% This is first transformed to the problem
% min_{L1,L2,S1,S2,Z}
%      1_{rank(L) <= r} + lambda * ||S1||_1 + mu * ||Z||_F
% s.t. L1 = L2
%      S1 = S2
%      Z = P_obs[ D - L2 - S2].
